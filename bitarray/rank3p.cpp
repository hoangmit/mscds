...
void Rank3pBuilder::build(const BitArray& b, Rank3p * o) {
    o->bits = b;
    uint64_t num_of_l1_l2 = b.length() / 2048;
    num_of_l1_l2 = b.length() % 2048 != 0 ? num_of_l1_l2 + 1: num_of_l1_l2;
    o->l1_l2=BitArray::create(num_of_l1_l2 * 64);
    o->l1_l2.fillzero();
    uint64_t num_of_l0 = b.length() / (1ULL << 32);
    num_of_l0 = b.length() % (1ULL << 32) != 0 ? num_of_l0 + 1 : num_of_l0;
    ...
    uint64_t num_of_sampling = b.length() / 8192;
    num_of_sampling = b.length() % 8192 != 0 ? num_of_sampling + 1 : num_of_sampling;
    o->sampling=BitArray::create(num_of_sampling * 32);
    uint64_t curr_l0_count, curr_sampling_idx=0, l0_sampling_offset, l0_blk=0, sampling_scale, curr_num_of_sampling;
    ...
    for (uint64_t i=0; i < o->l0.word_count(); ++i) {
        ...
        curr_num_of_sampling = (i + 1) * 524288 <= num_of_sampling ? 524288 : num_of_sampling % 524288;
        sampling_scale = curr_l0_count / curr_num_of_sampling;
        sampling_scale = sampling_scale == 0 ? 1 : sampling_scale;

        for (uint64_t j=1; j<=curr_l0_count && j<=curr_num_of_sampling*sampling_scale; j+=sampling_scale) {
            if (i*2097152+2097151 < o->l1_l2.word_count())
                l0_blk=o->l1binarysearch(j, l0_blk, i*2097152+2097151);
            else
                l0_blk=o->l1binarysearch(j, l0_blk, o->l1_l2.word_count()-1);

            o->sampling.setbits(curr_sampling_idx + l0_sampling_offset, l0_blk % 2097152, 32);
...
uint64_t Rank3p::basicblkcount(uint64_t blk) const {
    ...
    uint64_t num_of_l1_block = bits.length() / 512;
    num_of_l1_block = bits.length() % 512 != 0 ? num_of_l1_block + 1 : num_of_l1_block;

    if (blocknum == 3) {
        if (blk == num_of_l1_block - 1)
...
uint64_t Rank3p::select(const uint64_t r) const {
    ...
    uint64_t num_of_sampling = sampling.length() / 32;
    uint64_t curr_num_of_sampling = (l0_blk + 1) * 524288 <= num_of_sampling ? 524288 : num_of_sampling % 524288;
    uint64_t sampling_scale = curr_l0_count / curr_num_of_sampling;
    sampling_scale = sampling_scale == 0 ? 1 : sampling_scale;
    uint64_t block_of_scale=(i-1)/sampling_scale;
    block_of_scale = block_of_scale >= curr_num_of_sampling ? curr_num_of_sampling - 1 : block_of_scale;
    uint64_t l1_blk=sampling.bits((l0_blk*524288+block_of_scale)*32, 32)+(l0_blk*2097152); //Get L1 index from nearest sampling answer
    l1_blk+=(i-l1rank(l1_blk)-1)/2048; //Skip over L1 blocks if the difference in values is greater than 1 L1 block size

    if (sampling_scale > 1) {
        if (block_of_scale < curr_num_of_sampling - 1)
            l1_blk=l1binarysearch(i, l1_blk, sampling.bits((l0_blk*524288+block_of_scale+1)*32, 32) + (l0_blk*2097152));
        else {
            if (l0_blk*2097152+2097151 < l1_l2.word_count()) //Binary search to the correct L1 index
    ...
    uint64_t num_of_basic_blk = bits.length() / 512;
    num_of_basic_blk = bits.length() % 512 != 0 ? num_of_basic_blk + 1 : num_of_basic_blk;
