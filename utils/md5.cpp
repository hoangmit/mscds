...
void MD5::update(const uint1 *input, uint4 input_length) {
  ...
  if (finalized){  // so we can't update!
    throw std::runtime_error("MD5::update:  Can't update a finalized digest!");
    return;
  }
...
void MD5::update(const std::string& input) {
	update((const uint1 *)(input.c_str()), input.length());
}
...
std::vector<unsigned char> MD5::raw_digest(){
  
  if (!finalized)
	  throw std::runtime_error("MD5::raw_digest:  Can't get digest if you haven't"
		" finalized the digest!");
  std::vector<uint1> s(16);
...
std::string MD5::hex_digest(){
  ...
  for (i = 0; i < 16; i++) {
	  s[i * 2] = hexchar[(digest[i] >> 4)];
	  s[i * 2 + 1] = hexchar[digest[i] & 15];
